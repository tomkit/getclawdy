...
struct ResearchManifestEntry: Codable, Equatable {
    ...
    /// The SKILL that produced the run (`"research"` for the built-in one, the id of a
    /// Clawdy skill from `~/.clawdy/skills`, or a harness skill's name). Optional and
    /// default-nil so pre-existing manifests still decode; an absent value means the
    /// built-in research skill. Descriptive only — it never changes reconstruction.
...
final class ResearchManifestStore: @unchecked Sendable {
    ...
    func recordResearchSessionStarted(
        ...
        skillID: String = ClawdySkill.builtInResearchID
    ) {
