...
enum BuddyPushToTalkShortcut {
    ...
    /// Virtual keycode for the Escape key. Escape is an escape-hatch out of a
    /// wedged annotation mode (see `CompanionManager.handleEscapeKeyPressed`);
    /// when annotation mode is off it is completely inert.
    ...
    /// Whether the current push-to-talk modifier chord (e.g. `[.control, .option]`)
    /// is fully present in the given raw modifier flags. Used to reconcile the
    /// persisted pressed-state against the LIVE hardware modifier flags after the
    /// event tap is force-re-enabled. The raw value is interpreted as
    /// `NSEvent.ModifierFlags` device-independent bits — the same interpretation
    /// the tap already uses for `CGEventFlags.rawValue`, whose bit layout matches.
    static func modifierFlagsContainCurrentShortcut(modifierFlagsRawValue: UInt64) -> Bool {
        guard let modifierOnlyFlags = currentShortcutOption.modifierOnlyFlags else { return false }
        let liveFlags = NSEvent.ModifierFlags(rawValue: UInt(modifierFlagsRawValue))
            .intersection(.deviceIndependentFlagsMask)
        return liveFlags.contains(modifierOnlyFlags)
    }

    /// Reconciles the persisted pressed-state against the live hardware modifier
    /// flags. When a heavy-input window (e.g. a window-manager shortcut grabbing
    /// ctrl + option + arrow) force-disables the listen-only event tap, the user's
    /// modifier key-UP `flagsChanged` can be LOST — the tap never sees the release,
    /// so `wasPressed` stays stuck true and annotation mode wedges on. On tap
    /// re-enable we compare: if we believed the chord was held but the live flags
    /// no longer contain it, synthesize the missed `.released`.
    ///
    /// Returns `.released` for the missed-release case and `nil` otherwise (still
    /// genuinely held, or was never pressed). Pure so it's unit-testable with no tap.
    static func reconciledTransition(
        wasPressed: Bool,
        liveFlagsContainShortcut: Bool
    ) -> ShortcutTransition? {
        guard wasPressed, !liveFlagsContainShortcut else { return nil }
        return .released
    }

    /// Pure mapping: is this event an Escape key-down? Kept here (not inline in the
    /// tap) so the keyCode-53 → escape-cancel signal is unit-testable with no tap.
    static func isEscapeKeyDown(eventType: CGEventType, keyCode: UInt16) -> Bool {
        eventType == .keyDown && keyCode == escapeKeyCode
    }

    /// Whether this is one of the two events macOS delivers when it force-disables
    /// the tap (`.tapDisabledByTimeout` / `.tapDisabledByUserInput`). BOTH must
    /// route through the same re-enable + live-flag reconcile — a heavy-input
    /// window (the wedge trigger) surfaces as `.tapDisabledByUserInput`, while a
    /// slow callback surfaces as `.tapDisabledByTimeout`. Pure so the "both reasons
    /// reconcile" contract is unit-testable with no tap.
    static func isTapDisableEvent(_ eventType: CGEventType) -> Bool {
...
@MainActor
// Not `final` so tests can inject a spy subclass that records whether an abort
// took the cancel-WITHOUT-submit path vs the normal submitting release path.
class BuddyDictationManager: NSObject, ObservableObject {
