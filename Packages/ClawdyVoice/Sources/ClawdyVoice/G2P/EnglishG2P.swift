...
final public class EnglishG2P {
  ...
  private func tokenize(preprocessedText: PreprocessTuple) -> [MToken] {
    ...
    mutableTokens = EnglishG2P.splitTrailingSentencePunctuation(mutableTokens)
    mutableTokens = EnglishG2P.mergeIntraWordHyphens(mutableTokens)
  ...
  /// When the next sentence starts lowercase ("i'm clawdy. ask me"), NLTagger keeps the period
  /// glued to the word ("clawdy.") as if it were an abbreviation, which both loses the pause and
  /// feeds the "." to the fallback network. Split a single trailing `.`/`?`/`!` off a plain word
  /// into its own sentence-terminator token; dotted abbreviations ("e.g.", "U.S.") are left alone.
  static func splitTrailingSentencePunctuation(_ tokens: [MToken]) -> [MToken] {
    var result: [MToken] = []
    for token in tokens {
      guard token.text.count > 1,
            let last = token.text.last, ".?!".contains(last),
            !token.text.dropLast().contains(where: { ".?!".contains($0) }),
            token.text.dropLast().contains(where: { $0.isLetter || $0.isNumber }) else {
        result.append(token)
        continue
      }
      let wordEnd = token.tokenRange.upperBound
      let punctuationStart = token.text.index(before: token.text.endIndex)
      let word = MToken(text: String(token.text[..<punctuationStart]), tokenRange: token.tokenRange, tag: token.tag, whitespace: "")
      let punctuation = MToken(text: String(last), tokenRange: wordEnd..<wordEnd, tag: .sentenceTerminator, whitespace: token.whitespace)
      result.append(word)
  ...
  /// the way misaki expects (the "-" becomes silent junk instead of a spoken dash).
  static func mergeIntraWordHyphens(_ tokens: [MToken]) -> [MToken] {
    var merged: [MToken] = []
    var index = 0
    while index < tokens.count {
      let token = tokens[index]
      var runEnd = index
      // Extend over `- word` pairs glued to the current word with no whitespace on either side.
      while runEnd + 2 < tokens.count,
            tokens[runEnd].whitespace.isEmpty,
            tokens[runEnd + 1].text == "-",
            tokens[runEnd + 1].whitespace.isEmpty,
            tokens[runEnd].text.contains(where: { $0.isLetter || $0.isNumber }),
            tokens[runEnd + 2].text.contains(where: { $0.isLetter || $0.isNumber }) {
        runEnd += 2
      }
      if runEnd == index {
        merged.append(token)
      ...
      let compound = MToken(
        text: run.map { $0.text }.joined(),
        tokenRange: run.first!.tokenRange.lowerBound..<run.last!.tokenRange.upperBound,
        tag: run.first!.tag,
        whitespace: run.last!.whitespace
      )
    ...
    if let unk {
      ...
      for token in tokens {
        if token._.prespace,
           ...
           token.phonemes?.isEmpty == false {   // Python: `and tk.phonemes` — "" adds no space
          phonemeBuilder += " "
        }
  ...
  /// True for a token NLTagger may not have tagged `.number` but which reads as one ("12.50", "1,024").
  static func looksLikeNumber(_ token: MToken) -> Bool {
    if token.tag == .number { return true }
    return token.text.contains(where: { $0.isNumber })
    ...
    for (i, token) in tokens.enumerated() {
      ...
      for j in 0..<subtokens.count {
        ...
        if token.`_`.alias != nil || token.phonemes != nil {
          ...
          token.phonemes = "—"
          token.`_`.rating = 3
        } else if let tag = token.tag, EnglishG2P.punctuationTags.contains(tag), Lexicon.symbolSet[token.text] == nil, !token.text.lowercased().unicodeScalars.allSatisfy({ (97...122).contains(Int($0.value)) }) {
          ...
          // NLTagger tags a decimal amount ("12.50") as OtherWord rather than Number, so the
          // amount is recognised by its text, not only its tag, to keep the currency attached.
          if !EnglishG2P.looksLikeNumber(token) {
            currency = nil
          } else if j + 1 == subtokens.count && (i + 1 == tokens.count || !EnglishG2P.looksLikeNumber(tokens[i + 1])) {
