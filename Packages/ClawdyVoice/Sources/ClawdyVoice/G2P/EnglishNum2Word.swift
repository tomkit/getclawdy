...
struct EnglishNum2Word {
  ...
  private func toDecimal(_ number: Decimal) -> String {
    ...
    // The digits after the decimal point, read from the exact decimal description ("0.05" → "05").
    let fractionalDescription = "\(fractionalPart)"
    let fractionalString = fractionalDescription.contains(".") ? fractionalDescription.split(separator: ".", maxSplits: 1)[1] : Substring("")
