  ...
  private func getSpecialCase(_ word: String, tag: NLTag?, stress: Double?, ctx: TokenContext) -> (phoneme: String?, rating: Int?) {
    if tag == .punctuation, let target = Lexicon.addSymbols[word] {
      ...
      return lookup(sym, tag: nil, stress: nil, ctx: ctx)
              ...
              word.replacingOccurrences(of: ".", with: "").allSatisfy({ $0.isLetter }) {
      // Dotted abbreviations ("e.g.", "U.S.") are spelled out letter by letter. The
      // letters-only guard (as in Python misaki) keeps decimals like "4.2" on the number path.
        ...
        if isPlainDigits(word) {
          ...
          if let n = Int(num) {
            ...
            if num.first == "." {
              ...
              // Parse the digits exactly: `Decimal(Double)` carries binary-float noise
              // ("4.2" → 4.2000000000000002) that broke the fractional-digit expansion.
