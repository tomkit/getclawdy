...
- **Instant feedback, all in Clawdy's voice.** About a second after you release the keys Clawdy says "let me check" / "let me take a look" (a natural beat, not the instant you stop), and if the answer is taking a while it says "still working on it, hang tight" at the twenty-second mark (the cursor's "thinking…" pill covers the stretch in between) — the register of a voice assistant, not a status line. Research runs now say "sure, I'll put a page together", "your page is ready", and "sorry, that one didn't work out" instead of playing system sounds. There are no sound effects, and no two voice outputs ever overlap: a research announcement waits until you and Clawdy are both quiet.
